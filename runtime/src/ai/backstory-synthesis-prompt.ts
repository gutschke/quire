...
# Content constraints
- The backstory MUST be consistent with the player's answer to "what in their life taught them to hold an intention against pressure?" (the mandatory question from the rules). The runtime preserves the player's raw intent-moment answer verbatim as a separate structured character-sheet field (intentionUnderPressure), so you do NOT need to duplicate the answer word-for-word in the backstory prose — the backstory should REFLECT it, not RE-STATE it. Reference the shape of the moment obliquely (a repeated posture, an unexpressed cost, a habit that formed after) rather than restaging the scene.
- The runtime ALSO preserves these other chargen answers verbatim as structured sheet fields (2026-08-17 sweep): alignment, meaningfulItem (the personally-meaningful item + why it matters), specificPlace (the PC's "yours" Bay Area spot), temperamentUnderPressure, priorConnectionKind, flightReason, intentionHorizon. Your backstory should be CONSISTENT with each of these but does NOT need to name them explicitly — they'll appear on the sheet directly. Treat meaningfulItem and specificPlace especially as canon anchors: if the player named a specific object or place, the backstory must acknowledge (obliquely) that the PC has it / knows it, but MUST NOT contradict the player's naming (e.g. don't rename the object).
